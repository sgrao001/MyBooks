...
<head>
    ...
    <style>
        ...
        .tooltip-group .tooltip-trigger {{ display: inline-block !important; width: auto !important; margin: 0 4px !important; }}
        .tooltip-trigger.tooltip-full {{ height: auto; width: auto; }}
        ...
        #tooltip {{
            ...
            /* Padding (keep tooltip’s original for comfortable reading) */
        ...
        #tooltip.fade {{ transform: translateY(15px) scale(0.95); }}
        #tooltip.fade.show {{ transform: translateY(0) scale(1); }}

        #tooltip.bounce {{ transform: translateY(40px) scale(0.6); }}
        #tooltip.bounce.show {{ animation: bounceIn 0.6s cubic-bezier(0.68, -0.55, 0.265, 1.55); }}

        #tooltip.slide {{ transform: translateY(60px); }}
        #tooltip.slide.show {{ transform: translateY(0); }}

        #tooltip.zoom {{ transform: scale(0.4); }}
        #tooltip.zoom.show {{ transform: scale(1); }}

        #tooltip.left-text {{ text-align: left; }}
        #tooltip.right-text {{ text-align: right; }}
        #tooltip.center-text {{ text-align: center; }}
        ...
        #tooltip::-webkit-scrollbar-thumb {{ background: rgba(255,255,255,0.4); border-radius: 3px; }}
        ...
        .slider-container {{
            ...
            width: var(--page-width);
            height: 40px;
            /* New glass-morphism styles (matching #tooltip) */
            background: rgba(0, 0, 0, 0.45) !important;
            backdrop-filter: blur(15px) !important;
            -webkit-backdrop-filter: blur(15px) !important;
            border: 1px solid rgba(255, 255, 255, 0.2);
        ...
        .slider-info {{
            color: #b0e0ff;            /* matches tooltip text color */
            font-size: 0.8em;
            font-family: Georgia, 'Times New Roman', Times, serif;
        }}
    ...
    <script>
        ...
        document.addEventListener('click', function(e) {
            try {
                ...
                // If tooltip is open and clicking outside, close it
                if (tooltip.classList.contains('show') && !trigger && !tooltip.contains(e.target)) {
                    toggleTooltip('', '', '', '', '', 'no');
                    return;
                }
                ...
                trigger.classList.forEach(cls => {
                    if (cls !== 'tooltip-trigger' && !cls.startsWith('left-justified') && !cls.startsWith('right-justified') && !cls.startsWith('center-justified')) {
                ...
                if (position !== 'absolute') {
                    let tooltipWidth = parseFloat(trigger.dataset.width);
                    let tooltipHeight = parseFloat(trigger.dataset.height);
                    let totalTooltipWidth = tooltipWidth ? tooltipWidth + extra : null;
                    let totalTooltipHeight = tooltipHeight ? tooltipHeight + extra : null;

                    if (top === 'center') {
                        if (totalTooltipHeight) {
                            let centerTop = (window.innerHeight / 2) - (totalTooltipHeight / 2);
                            centerTop = Math.max(padding, Math.min(centerTop, window.innerHeight - totalTooltipHeight - padding));
                            top = centerTop + 'px';
                        } else {
                            top = '50%';
                        }
                    ...
                    if (left === 'center') {
                        if (totalTooltipWidth) {
                            let centerLeft = (window.innerWidth / 2) - (totalTooltipWidth / 2);
                            centerLeft = Math.max(padding, Math.min(centerLeft, window.innerWidth - totalTooltipWidth - padding));
                            left = centerLeft + 'px';
                        } else {
                            left = '50%';
                        }
                ...
                const rawTiptext = trigger.dataset.tiptext || 'Default tooltip';
                const animation = trigger.dataset.animation || 'fade';
                const textAlign = trigger.dataset.textalign || 'left';
                const tiptext = htmlEncodeTooltipText(rawTiptext);

                const isCurrentlyOpen = tooltip.classList.contains('show') && tooltip.innerHTML.includes(rawTiptext);

                if (isCurrentlyOpen) {
                    toggleTooltip('', '', '', '', '', 'no');
                } else {
                    const showTooltip = () => {
                        toggleTooltip(top, left, height, width, tiptext, 'yes', animation, textAlign, position, extraClasses, bottom, maxHeight, bottomGap, trigger, right);
                        // Force layout recalculation after the tooltip is visible
                    ...
                    if (document.fonts) {
                        document.fonts.ready.then(showTooltip);
                    } else {
                        showTooltip();
                    }
                }
      ...
      // ==================== TOOLTIP inside click close end ====================
        ...
        // htmlEncodeTooltipText (keep your existing version)
        function htmlEncodeTooltipText(str) {
            const div = document.createElement('div');
            div.innerHTML = str;
            return div.innerHTML;
        }
        function toggleTooltip(top, left, height, width, tiptext, visible, animation = 'fade', textAlign = 'left', position = 'fixed', extraClasses = [], bottom = undefined, maxHeight = null, bottomGap = null, trigger = null, right = undefined) {
            const tooltip = document.getElementById('tooltip');

            if (visible === 'yes') {
                // --- Prepare tooltip for display ---
                tooltip.className = '';
                extraClasses.forEach(cls => tooltip.classList.add(cls));
                tooltip.style.setProperty('position', position, 'important');
                if (textAlign !== 'left') tooltip.classList.add(textAlign + '-text');
                if (animation !== 'fade') tooltip.classList.add(animation);
                else tooltip.classList.add('fade');
                ...
                if (width !== 'auto') tooltip.style.width = width;
                else tooltip.style.width = '';
                if (height !== 'auto') tooltip.style.height = height;
                else tooltip.style.height = '';

                // Apply max‑width / max‑height (dataset overrides computed maxHeight)
                if (datasetMaxWidth) tooltip.style.maxWidth = datasetMaxWidth;
                else tooltip.style.maxWidth = '';
                if (datasetMaxHeight) tooltip.style.maxHeight = datasetMaxHeight;
                else if (maxHeight !== null) tooltip.style.maxHeight = maxHeight + 'px';
                else tooltip.style.maxHeight = '';
                ...
                if (needsLayout) {
                    // Make tooltip occupy space but invisible while we position it
                    ...
                    requestAnimationFrame(() => {
                        requestAnimationFrame(() => {
                            ...
                            if (top === '@' && trigger) {
                                ...
                                tooltip.removeAttribute('style');  // clear all inline styles
                                tooltip.style.cssText = '';
                                // Set our styles
                                tooltip.style.setProperty('width', '50vw', 'important');
                                tooltip.style.setProperty('max-width', '50vw', 'important');
                                tooltip.style.setProperty('max-height', '3em', 'important');
                                ...
                                tooltip.style.setProperty('transition', 'all 0.5s cubic-bezier(0.65, 0, 0.35, 1)');
                                tooltip.style.setProperty('background', 'rgba(0, 0, 0, 0.45)');
                                tooltip.style.setProperty('backdrop-filter', 'blur(15px)');
                                tooltip.style.setProperty('border-radius', '8px');
                                tooltip.style.setProperty('box-shadow', '0 5px 5px rgba(0, 0, 0, 0.95)');
                                tooltip.style.setProperty('border', '1px solid rgba(255, 255, 255, 0.2)');
                                tooltip.style.setProperty('padding', '10px');
                                tooltip.style.setProperty('font-size', '0.8em');
                                tooltip.style.setProperty('color', '#b0e0ff');
                                tooltip.style.setProperty('font-family', 'Arial, sans-serif');
                                tooltip.style.setProperty('z-index', '9999');
                                tooltip.style.setProperty('white-space', 'normal');
                                tooltip.style.setProperty('word-wrap', 'break-word');

                                // Force layout to get accurate dimensions after width change
                                const tooltipHeightNow = tooltip.offsetHeight;
                                const tooltipWidthNow = tooltip.offsetWidth;
                                console.log('Computed width after setProperty:', tooltipWidthNow);
                                console.log('Computed height after setProperty:', tooltipHeightNow);
                                ...
                                if (triggerCenterX < threshold25) {
                                    ...
                                    leftPos = viewportWidth - tooltipWidthNow - rightMargin;   // 5vw from right edge
                                    console.log('Case: right (trigger center > 75vw)');
                                } else {
                                    leftPos = (viewportWidth / 2) - (tooltipWidthNow / 2);     // centered
                                    console.log('Case: center');
                                }

                                // Ensure tooltip stays within viewport vertically
                                if (topPos + tooltipHeightNow > viewportHeight) {
                                    topPos = viewportHeight - tooltipHeightNow - bot_gap;
                                ...
                                leftPos = Math.max(10, Math.min(leftPos, viewportWidth - tooltipWidthNow - 10));
                            ...
                            if (!positioned) {
                                ...
                                if (pos === 'center-height' || pos === 'center-both') {
                                    ...
                                    if (pageElement) {
                                        ...
                                        finalTop = contentTop + (contentHeight / 2) - (tooltipHeight / 2);
                                    } else {
                                        finalTop = (window.innerHeight / 2) - (tooltipHeight / 2);
                                    }
                                }

                                // Horizontal centering (center-width or center-both)
                                if (pos === 'center-width' || pos === 'center-both') {
                                    const tooltipWidthNow = tooltip.getBoundingClientRect().width;
                                    if (pageElement) {
                                        const pageRect = pageElement.getBoundingClientRect();
                                        const cs = getComputedStyle(pageElement);
                                        ...
                                        finalLeft = contentLeft + (contentWidth / 2) - (tooltipWidthNow / 2);
                                    } else {
                                        finalLeft = (window.innerWidth / 2) - (tooltipWidthNow / 2);
                                    }
                                    // For center-width only, keep any existing top/bottom
                                    if (pos === 'center-width') {
                                        if (bottom !== undefined) tooltip.style.bottom = bottom;
                                        else if (top !== undefined && top !== '@') tooltip.style.top = top;
                                    }
                                }

                                // For center-both, ensure width/height are auto (remove inline) ONLY if they were not explicitly set
                                if (pos === 'center-both') {
                                    if (width === 'auto') tooltip.style.width = '';
                                    if (height === 'auto') tooltip.style.height = '';
                                }
                                // --- Bottom positioning modes ---
                                    ...
                                    if (pageElement) {
                                        const pageRect = pageElement.getBoundingClientRect();
                                        newTop = pageRect.bottom - tooltipHeight - gapPx;
                                        newTop = Math.max(pageRect.top + 10, newTop);
                                    } else {
                                        newTop = window.innerHeight - tooltipHeight - gapPx;
                                    ...
                                    if (pos === 'bottom-left') {
                                        ...
                                        if (pageElement) {
                                            const pageRect = pageElement.getBoundingClientRect();
                                            newLeft = pageRect.right - tooltipWidthNow - innerPadding;
                                        } else {
                                            newLeft = window.innerWidth - tooltipWidthNow - innerPadding;
                                        }
                                    } else if (pos === 'bottom-center') {
                                        if (pageElement) {
                                            const pageRect = pageElement.getBoundingClientRect();
                                            newLeft = (pageRect.left + pageRect.right) / 2 - tooltipWidthNow / 2;
                                        } else {
                                            newLeft = (window.innerWidth / 2) - (tooltipWidthNow / 2);
                                        }
                                        // Clamp to page/viewport edges
                                        if (pageElement) {
                                            const pageRect = pageElement.getBoundingClientRect();
                                            newLeft = Math.max(pageRect.left + innerPadding, Math.min(newLeft, pageRect.right - tooltipWidthNow - innerPadding));
                                        } else {
                                            newLeft = Math.max(innerPadding, Math.min(newLeft, window.innerWidth - tooltipWidthNow - innerPadding));
                                        }
                            ...
                            if (finalTop !== null) tooltip.style.top = (typeof finalTop === 'number' ? finalTop + 'px' : finalTop);
                            if (finalBottom !== null) tooltip.style.bottom = (typeof finalBottom === 'number' ? finalBottom + 'px' : finalBottom);
                            if (finalLeft !== null) tooltip.style.left = (typeof finalLeft === 'number' ? finalLeft + 'px' : finalLeft);
                            if (finalRight !== null) tooltip.style.right = (typeof finalRight === 'number' ? finalRight + 'px' : finalRight);

                            if (finalBottom !== null && finalTop === null) tooltip.style.top = 'auto';
                            if (finalRight !== null && finalLeft === null) tooltip.style.left = 'auto';

                            // Now that the tooltip is positioned correctly, make it visible
                ...
                // --- Hide tooltip: fade out without repositioning ---
...
def pre_clean(content):
    ...
    # convert LINKS from URL [] () to <a href="url>test</a>
    content = re.sub(r'\[([^\]]+)\]\s*\(([^)]+)\)', r'<a href="\2" target="_blank" rel="noopener noreferrer">\1</a>', content, flags=re.MULTILINE)

    # Replace <STARTPAGE>, <NEWPAGE>< <ENDPAGE> with <div style="break-after: page;"></div><br>
    content = re.sub( r'<\s*(?:firstpage|newpage|lastpage)\s*>', '<div style="break-after: page;"></div><br>', content, flags=re.IGNORECASE | re.MULTILINE )

    # replace <CHAPTER="xxx"> with ## xxx also support 'xxx' there must be a space between ## and xxx
    content = re.sub( r'<\s*chapter\s*=\s*(["\'])(.*?)\1\s*>', r'## \2', content, flags=re.IGNORECASE )

    # replace <IMAGE="xxx"> with ![[xxx]] and also support 'xxx'
    content = re.sub( r'<\s*image\s*=\s*(["\'])(.*?)\1\s*>', r'![[\2]]', content, flags=re.IGNORECASE )

    # Replace <TITLE="xxx"> with <p style="color: #f09e5a;"><span class="highlight-text">Fragility</span></p>xxx</span></p>
    content = re.sub(r'<\s*title\s*=\s*(["\'])(.*?)\1\s*>', r'<p style="color: #f09e5a;"><span class="highlight-text">\2</span></p>', content, flags=re.IGNORECASE )

    # convert <SOURCE="label","URL"> 
    content = re.sub( r'<\s*source\s*=\s*(["\'])(.*?)\1\s*,\s*(["\'])(.*?)\3\s*>', r'<span style="font-size: 0.8em;"><a href="\4">\2</a></span>', content, flags=re.IGNORECASE | re.DOTALL)
    content = re.sub( r'<\s*source\s*=\s*(["\'])(.*?)\1\s*>', r'<span style="font-size: 0.8em;"><a href="\2">Image Src...</a></span>', content, flags=re.IGNORECASE | re.DOTALL)

    # replace <FOOTER="xxx">
    content = re.sub( r'<\s*FOOTER\s*=\s*(["\'])(.*?)\1\s*>', r'<span style="font-size: 0.8em;">\2</span>', content, flags=re.IGNORECASE )
    ...
    def replace_dposition(match):
        # Extract the parameters string between :[ and ]
        ...
        # Text-align style (default left)
        talign = t_raw if t_raw else 'left'
        text_align = f'text-align: {"left" if talign == "left" else "right" if talign == "right" else "center"} !important; '

        # Font style (default normal)
        fstyle = f_raw if f_raw else 'normal'
        font_style = f'font-style: {fstyle} !important;'

        # Assemble final div
        return f'<div style="{margin}{text_align}{font_style}">{content_text}</div>'

    content = re.sub(
        r'<\s*DPosition\s*=\s*(["\']):\[(.*?)\](.*?)\1\s*>',
        replace_dposition,
        content,
    ...
    def build_flexbot_span(match, float_dir):
        ...
        attrs += f' data-tiptext="{tip_text}"'

        classes = 'tooltip-trigger tooltip-full tooltip-flex'
